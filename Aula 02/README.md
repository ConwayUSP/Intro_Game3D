...
Ufa, isso foi longo, agora está na mora de ligar essa shader com o resto da cena.
...
Nosso script é simples, nós atrelamos a função `_on_body_entered` ao sinal `body_entered` e caso o corpo tenha um metadado chamado `teleportable` com o valor `true` nós teleportamos o corpo até a posição de saída do portal gêmeo.
...
func shoot_portal(type: String):
	if raycast.is_colliding():
		...
		# Se o portal já estiver posicionado, "retraia" ele de volta
...
func place_portal(portal, point, normal):
	...
	# Alinha o portal com a direção que a parede aponta (sua normal)
	# A frente de portal será a mesma direção que a parede aponta
	if normal.is_equal_approx(Vector3.UP):
		portal.look_at(point - normal, Vector3.FORWARD)
	else:
		portal.look_at(point - normal, Vector3.UP)
	
	# Pequena diferença para impedir que o portal fique dentro da parede
	portal.global_position += normal * 0.05
```

Por fim, a função `place_portal` é ainda mais simples. Nós ativamos o portal, setando `visible` para `true` e posicionamos ele contra a superfície da parede, apontando a saída do nosso portal para a mesma direção que a parede, ou teto apontam, fazendo um ajusto de posição para que o portal fique **sobre** a superfície e não **dentro**.
...
Chegamos ao final do nosso curso! Meus parabéns, vá para o projeto final para saber quais são as próximas etapas.
